...
const RoutesTabsHeadquater = props => {
    ...
    const routeFetchIntelligence =
        `${process.env.OPEN_VALLE_MAGICO_URL}intelligences/headquarter`;
    const routeFetchStyles = `${process.env.OPEN_VALLE_MAGICO_URL}styles/headquarter`;

    const routeFetchIntelligenceCompetitions =
        `${process.env.OPEN_VALLE_MAGICO_URL}competences/intelligences/headquarter`;
    const routeFetchStyleCompetitions =
        `${process.env.OPEN_VALLE_MAGICO_URL}competences/styles/headquarter`;

    const routeFetchVocational =
        `${process.env.OPEN_VALLE_MAGICO_URL}vocationals/headquarter`;
    const routerFetchRecomendationSubject =
        `${process.env.OPEN_VALLE_MAGICO_URL}recomendations/headquarter`;

    const routeFetchGamesPlayed = `${process.env.OPEN_VALLE_MAGICO_URL}gamesPlayed/headquarter`

    const fetchHierarchy = `${process.env.OPEN_VALLE_MAGICO_URL}api/groupsByHeadquarters`;

    const fetchTitleName = `${process.env.OPEN_VALLE_MAGICO_URL}api/headquarter`;
    ...
    useEffect(() => {
        (async function () {
            ...
            const id = await fetchApi(fetchHierarchy);
        ...
        <div className="col-12 m-0">
            ...
            <Switch>
                ...
                <Route path={`${url}/recomendaciones`}>
                    <ViewRecomendations
                        urlToFetchInfoRecomendationSubject={`${routerFetchRecomendationSubject}/${params.idForFetch}`}
