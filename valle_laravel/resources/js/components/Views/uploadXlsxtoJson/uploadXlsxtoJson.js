...
const UploadXlsxtoJson = () => {
    ...
    function keysToLowerCase(obj) {
        ...
        console.log("TCL: keysToLowerCase -> obj", obj)
        ...
        obj.map(
            (item) => {
                ...
                if (parseInt(item.COD_GRADO) >= 0 && parseInt(item.COD_GRADO) <= 9) {

                    let doc;

                    if (isNaN(parseInt(String(item.NRO_IDENTIDAD).replace(/[^0-9]/g, ""))) == false) {
                        doc = parseInt(String(item.NRO_IDENTIDAD).replace(/[^0-9]/g, ""));
                    } else {
                        doc = 11111111;
                    }
                    ...
                    setUsersJson({
                        ...
                        ...UsersJson, [item.COD_GRADO]: UsersJson[item.COD_GRADO].push(
                            {
                                nombre_sede: String(detectUndefined(item.NOMBRE_SEDES)).trim().normalize("NFD").replace(/[^-A-Za-z0-9 ]/g, '').replace(/ /g, "_"),
                                nombre_institucion: String(detectUndefined(item.ESTABLECIMIENTO)).trim().normalize("NFD").replace(/[^-A-Za-z0-9 ]/g, '').replace(/ /g, "_"),
                                nombre_municipio: String(detectUndefined(item.MUNICIPIO)).trim().normalize("NFD").replace(/[^-A-Za-z0-9 ]/g, '').replace(/ /g, "_"),
                                zona: String(detectUndefined(item.ZONAS)).trim().normalize("NFD").replace(/[^-A-Za-z0-9 ]/g, '').replace(/ /g, "_"),
                                region: String(detectUndefined(item.Región)).trim().normalize("NFD").replace(/[^-A-Za-z0-9 ]/g, '').replace(/ /g, "_"),
                                doc: doc,
                                grado_cod: item.COD_GRADO,
                                apellido1: String(detectUndefined(item.PRI_APE)).trim().normalize("NFD").replace(/[^-A-Za-z0-9 ]/g, ''),
                                apellido2: String(detectUndefined(item.SEG_APE)).trim().normalize("NFD").replace(/[^-A-Za-z0-9 ]/g, ''),
                                nombre1: String(detectUndefined(item.PRI_NOM)).trim().normalize("NFD").replace(/[^-A-Za-z0-9 ]/g, ''),
                                nombre2: String(detectUndefined(item.SEG_NOM)).trim().normalize("NFD").replace(/[^-A-Za-z0-9 ]/g, ''),
        ...
        <div className='content-uploadxlsm col-md-12 d-flex align-items-center justify-content-center text-center'>
            <div>
                ...
                <input className='btn-uploadFile btn' type='submit'
                    ...
                    onClick={handleFile} />
                {
                    Object.keys(UsersJson).map(
                        (grade) =>
                            <div>
                                {
                                    UsersJson[grade].length > 0 &&
                                    <input type="button" value={`Grado ${grade}`} id={grade} onClick={handleClick} />
                                }
                            </div>
                    )
                }
