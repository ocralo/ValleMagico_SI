...
const ModalDelete = ({ modalId ,title, textBody, urlFetch , element }) => {
    ...
    async function deleteElement() {
        try {
            const data = await fetchPOST(`${urlFetch}/${element.id}`, element, 'DELETE')
            ...
            <div className="modal-dialog modal-sm" role="document">
                <div className="modal-content">
                    ...
                    <div className="modal-footer d-flex justify-content-between ">
                        ...
                        <button type="button" className="btn btn-modal-close" onClick={handleClick} data-dismiss="modal">Eliminar</button>
                    </div>
                </div>
            </div>
        </div>
